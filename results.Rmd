...
```{r stackbar, echo=FALSE}
g <- ggplot(joined, aes(negativity_binned))
g + geom_bar(aes(fill = race_ethn)) + scale_y_log10()
```

It's not really clear what a log scale on a stacked bar plot *shows.* It makes the 44 `NA` race patients (just 1 pat_id who wasn't in demographic table) look *huge* in number. Also it's telling me that I have $10^{15}$ patient notes, which is absurd!

```{r stackbar2, echo=FALSE}
ggplot(joined, aes(negativity_any)) +
  geom_bar(aes(fill = race_ethn)) +
  scale_y_log10()
```
