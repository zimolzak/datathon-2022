...
done_searching %>% select(-NOTE_TEXT) %>% head() %>% kable()
```
...
```{r}
table(joined$Sex, joined$negativity) %>% kable()
```

```{r}
table(joined$Ethnic.Group, joined$negativity) %>% kable()
```
```{r}
table(joined$Race, joined$negativity) %>% kable()
```
