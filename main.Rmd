...
sample_me %>%
  ...
  transmute(
    dispo = case_when(
      ED.Disposition == 'Admit' ~ 'adm',
      ED.Disposition == 'Discharge' ~ 'dis',
      ED.Disposition == 'Observation' ~ 'obs'
    ),
    clas = tolower(substr(Patient.Class, 1, 3)),
    ed_ad = ed_adm,
    ed_di = ed_dis,
    in_ad = inp_adm,
    in_di = inp_dis,
    ed_dur = round(ed_dur, 1),
    in_dur = round(inp_dur, 1),
